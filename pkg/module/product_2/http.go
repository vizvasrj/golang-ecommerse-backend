...
func GetProductBySlug(app *conf.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		query := `
		SELECT 
			id, sku, name, slug, image_url, image_key, description, quantity, price, taxable, is_active, brand_id, merchant_id, updated, created
		...
		err := app.DB.QueryRowContext(c, query, slug).Scan(
			...
			&product.Quantity, &product.Price, &product.Taxable, &product.IsActive, &product.BrandID, &product.MerchantID, &product.Updated, &product.Created)
...
func SearchProductsByName(app *conf.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		rows, err := app.DB.QueryContext(c, `
			SELECT id, sku, name, slug, image_url, description, quantity, price
			FROM products
			WHERE name ILIKE $1 AND is_active = TRUE
		`, "%"+productName+"%") // Case-insensitive search with ILIKE and wildcards
		...
		for rows.Next() {
			var product Product                                                                                                                                   // Use the product struct
			err := rows.Scan(&product.ID, &product.SKU, &product.Name, &product.Slug, &product.ImageURL, &product.Description, &product.Quantity, &product.Price) // Scan the columns you select
		...
		c.JSON(http.StatusOK, gin.H{"products": products}) // Return even if empty
