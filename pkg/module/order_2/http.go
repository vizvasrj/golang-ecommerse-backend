...
func AddOrderWithCartItemAndAddress(app *conf.Config) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		cartItems := []cart2.CartItem{} // Use cart2.CartItem
		...
		razorpayOrderID, err := payment.Executerazorpay(total, newReceiptID, newOrderID.String())
		...
		_, err = tx.ExecContext(ctx, `
			INSERT INTO receipts (id, order_id, amount, created, updated, payment_provider, razorpay_order_id)
			VALUES ($1, $2, $3, $4, $5, $6, $7)
		`, newReceiptID, newOrderID, total, time.Now(), time.Now(), "razorpay", razorpayOrderID)
		if err != nil {
			l.DebugF("Failed to insert receipt: %v", err) // Log error for debugging
		...
		if err := tx.Commit(); err != nil {
			...
			l.DebugF("Failed to commit transaction: %v", err)                                      // Log the error
...
func FetchOrder(app *conf.Config) gin.HandlerFunc { // Updated FetchOrder
	return func(c *gin.Context) {
		...
		orderInfo.Products = make([]cart2.CartItem, 0) // Initialize to empty slice
...
func UpdateItemStatus(app *conf.Config) gin.HandlerFunc { // Updated UpdateItemStatus
	return func(c *gin.Context) {
		...
		if status == cart2.Cancelled { // Use the enum from the correct package
			...
			`, req.CartID, cart2.Cancelled).Scan(&activeOrderItemsCount) // Use $1 and req.CartID.
