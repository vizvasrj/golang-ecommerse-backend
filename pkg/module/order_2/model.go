...
type OrderItem struct {
	...
	Status        cart2.CartItemStatus `db:"status" json:"status"` // Assuming CartItemStatus is defined similarly in cart2
...
type OrderInfo struct { // Struct for fetching additional details
	...
	Products []cart2.CartItem `json:"products"`
}
...
type UpdateOrderItemStatusRequest struct {
	...
	CartID  uuid.UUID            `json:"cart_id" binding:"required"`
	Status  cart2.CartItemStatus `json:"status"`
}
